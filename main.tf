...
resource "aws_route_table" "private" {
  count = "${var.create_vpc && local.max_subnet_length > 0 ? (var.single_nat_gateway ? 1 : local.max_subnet_length) : 0}"
...
resource "aws_eip" "nat" {
  count = "${var.create_vpc && (var.enable_nat_gateway && !var.reuse_nat_ips) ? (var.single_nat_gateway ? 1 : length(var.private_subnets)) : 0}"
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  count = "${var.create_vpc && var.enable_s3_endpoint ? (var.single_nat_gateway ? 1 : length(var.private_subnets)) : 0}"
...
resource "aws_vpc_endpoint_route_table_association" "private_dynamodb" {
  count = "${var.create_vpc && var.enable_dynamodb_endpoint ? (var.single_nat_gateway ? 1 : length(var.private_subnets)) : 0}"
